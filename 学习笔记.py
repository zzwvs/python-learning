...
print(Person)       # self-->实例对象本身
print(pe.slepp)     # cls-->类对象本身
'''
"""
|---------------------------------------------------------------------------------------------------
|  维度  |                     self                 |                        cls                    |
|---------------------------------------------------------------------------------------------------
|出现位置 |实例方法(Instance Method)的第一个参数      |类方法(@classmethod 装饰)的第一个参数            |
|---------------------------------------------------------------------------------------------------
|代表对象 |类的实例（对象）本身                       |类(Class)本身                                   |
|---------------------------------------------------------------------------------------------------
|访问权限 |可以访问实例属性、其他实例方法、类属性/类方法|可以访问类属性、其他类方法，但不能直接访问实例属性  |
|---------------------------------------------------------------------------------------------------
|调用方式 |通过实例调用(obj.method())                |通过类或实例调用(Class.method() 或 obj.method()) |
|---------------------------------------------------------------------------------------------------
"""
'''
class Myclass:
    class_attr = "我是类属性,未修改"
    def instance_(self):    # 实例方法,指向实例本身
        self.class_attr = "我是实例属性"    # 不会修改类属性,而是创建与类属性同名的实例属性
    @classmethod
    def class_(cls):    # 类方法,指向类本身
        cls.class_attr = "我是类属性,已修改"    # 修改实例属性

my = Myclass()
print(my.class_attr)
my.instance_()  # 创建一个实例属性,会遮盖同名的类属性
print(my.class_attr)
my.class_()
print(my.class_attr)    # python会优先查找实例属性,再查找类属性,所以这个print()不会输出"我是实例属性,已修改"
del my.class_attr   # 删除掉实例属性时就不会覆盖同名的类属性
print(my.class_attr)    # 成功输出"我是实例属性,已修改"
'''

# 小结:
# 实例方法能访问实例属性和类属性
# 静态方法@staticmethod:方法内部,不需要访问实例属性,如果要访问类属性就,通过类名.类属性名访问,不能访问实例属性
# 类方法@classmethod:方法内部只需要访问类属性,可以通过cls,类属性名访问类属性,不能访问实例属性
"""
单例模式:一种常用的软件设计模式,确保一个类在整个程序运行期间，只存在一个实例对象。
优点:可以节省空间
"""
# 装饰器写法
# def singleton(cls):
...
# 实例化对象时最先执行__new__(),__init__()被覆盖,不输出
# class Test():
#     def __init__(self):
#         print("这是__init__()")
...
#         return super().__new__(cls) # 写__new__()一定要执行这个,否则python得不到分配空间的引用,就不会调用__init__()
#         # 不能在类里返回实例化对象的引用
#         # return t1  不允许
# t1 = Test()
# print("t1:",t1)     # 结果与return返回值的引用一样

# 小结:
# __new__() 是创建对象,__init__() 是初始化对象
# __new__() 返回对象的引用,__init__() 是初始化实例属性
...
# print(a2)
# 内存地址不一样,说明创建了两个对象
# 单例模式:对象的内存地址一样;重写__new__()方法实现单例模式

"""
# 用__new__()实现单例模式,设计流程:
# 1.定义一个类属性,初始值为None,用来记录单例对象的引用(内存地址)
# 2.重写__new__()
# 3.判断:如果类属性是None,把__new__()返回的对象的引用保存进去
# 4.返回类属性中记录的对象引用
...
#     def __new__(cls,*a,**w):
#         print("这是__new__()方法")
#         if cls.obj == None:     # 判断并修改类属性
#             cls.obj = super().__new__(cls)
#         return cls.obj      # 再次调用返回旧的引用,实现单例模式
#     def __init__(self):
#         print("我是__new__()方法")
...
# a2 = A(3,6)
# print(a2)
# 虽然输出了16和18,但并不意味着创建了两个实例对象(它们的引用是相同的,只是值被覆盖)
"""
...
from 临时检验 import mokuai as m

m1 = m.get_mokuai(2,8)
m2 = m.get_mokuai(3,6)  # 返回的是上一个实例对象的返回值,不创建新的实例对象
print("==========")
m.A(1,4)
m.A(3,3)
# 以上调用的都只用一个模块,达到单例模式的目的,且比重写__new__()要安全(线程安全)
"""
"""
单例模式的应用场景:
1.回收站对象
...
魔法方法
"""
# 在python中,__xx__()就是魔法方法,如__new__()、__init__()
# 1.__doc__():类的描述信息
# class Person:
#     """人类--类的描述信息"""    # 只能使用使用多行注释,单行注释无效
#     pass
# def _Person():
#     """人类--函数的描述信息"""
#     pass

# print(Person.__doc__)   # 打印:人类--类的描述信息
# print(_Person.__doc__)  # 在函数中也能使用

# 2.__module__():表示当前操作对象所在的模块
# 3.__class__():表示当前对象所在的类
# 4.__str__():对象的描述信息
# 如果类中定义了此方法，那么在打印对象时，默认输出该方法的返回值--->此方法必须返回字符串
# 5.__del__():析构函数，在程序结束时会调用，或者删除某个对象时也会被调用
# 6.__call__():使一个实例对象变成一个可调用对象，像函数那样可以调用
# 可调用对象：方式后面可以有()的都是可调用对象
# 函数callable()可查看是否是可调用对象
# print(callable(range))  # 返回True
# class C:
#     def __call__(self, *args, **kwds):
#         print("这是__call__")

# a = C()
# a()     # 调用一个可调用的实例对象，其实是调用它的__call__()方法
# print(callable(a))
